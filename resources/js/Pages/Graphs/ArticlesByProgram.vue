...
const chartDataConfig = ref({
  labels: props.chartData.map(program => program.name),
  datasets: [{
    label: 'Artículos por Programa Académico',
    data: props.chartData.map(program => program.total),
    backgroundColor: props.chartData.map(program => getColorFromId(program.id, 0.5)),
    borderColor: props.chartData.map(program => getColorFromId(program.id, 1)),
    borderWidth: 1
  }]
});
...
const chartOptions = ref({
  ...
  plugins: {
    ...
    title: { 
      display: true, 
      text: 'Distribución de Artículos por Programas Académicos',
      font: {
        size: 16
      }
    },
...
async function downloadChartsAs(format = "pdf") {
  try {
    ...
    if (format === "pdf") {
      ...
      pdf.save("articulos_por_programa.pdf");
    } else {
      const link = document.createElement("a");
      link.href = imgData;
      link.download = `articulos_por_programa.${format}`;
      link.click();
    }
  } catch (error) {
...
<template>
  <HeadLogo title="Artículos por programa académico" />
  <LayoutAuthenticated>
    <SectionMain>
      <SectionTitleLineWithButton
        :icon="mdiChartTimelineVariant"
        title="Artículos Por Programa Académico"
        main
      />
      ...
      <div id="charts-pdf-content" class="p-6 bg-white dark:bg-gray-800 rounded-lg shadow relative">
        ...
        <div class="mt-8 overflow-x-auto shadow-md rounded-lg">
          ...
          <table class="min-w-full border border-gray-200 dark:border-gray-700">
            ...
            <tbody>
              <tr v-for="program in chartData" :key="program.id" 
                  :class="{'bg-gray-50 dark:bg-gray-700': program.id % 2 === 0, 'bg-white dark:bg-gray-800': program.id % 2 !== 0}">
              ...
              <tr class="bg-gray-100 dark:bg-gray-700 font-semibold">
                ...
                <td class="px-4 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100 border-b border-gray-200 dark:border-gray-700">
                  {{ chartData.reduce((sum, program) => sum + program.total, 0) }}
                </td>
              </tr>
            </tbody>
          </table>
