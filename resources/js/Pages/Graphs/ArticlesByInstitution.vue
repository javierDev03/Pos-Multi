...
const chartData = ref({
  labels: props.institutions.map(inst => inst.name),
  datasets: [{
    label: 'Artículos por Institución',
    data: props.institutions.map(inst => inst.articles_count),
    backgroundColor: props.institutions.map(inst => getColorFromId(inst.id, 0.5)),
    borderColor: props.institutions.map(inst => getColorFromId(inst.id, 1)),
    borderWidth: 1
  }]
});
...
const chartOptions = ref({
  ...
  plugins: {
    ...
    title: { 
      display: true, 
      text: 'Distribución de Artículos por Institución',
      font: {
        size: 16
      }
    },
...
async function downloadChartsAs(format = "pdf") {
  try {
    ...
    if (format === "pdf") {
      ...
      pdf.save("articulos_por_institucion.pdf");
    } else {
      const link = document.createElement("a");
      link.href = imgData;
      link.download = `articulos_por_institucion.${format}`;
      link.click();
    }
  } catch (error) {
...
<template>
  <HeadLogo title="Artículos por institución" />
  <LayoutAuthenticated>
    <SectionMain>
      <SectionTitleLineWithButton
        :icon="mdiChartTimelineVariant"
        title="Artículos Por Institución"
        main
      />
          ...
          <table class="min-w-full border border-gray-200 dark:border-gray-700">
            ...
            <tbody>
              <tr v-for="institution in institutions" :key="institution.id" 
                  :class="{'bg-gray-50 dark:bg-gray-700': institution.id % 2 === 0, 'bg-white dark:bg-gray-800': institution.id % 2 !== 0}">
              ...
              <tr class="bg-gray-100 dark:bg-gray-700 font-semibold">
                ...
                <td class="px-4 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100 border-b border-gray-200 dark:border-gray-700">
                  {{ institutions.reduce((sum, inst) => sum + inst.articles_count, 0) }}
                </td>
              </tr>
            </tbody>
          </table>
