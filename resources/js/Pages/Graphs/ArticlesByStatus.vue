...
const backgroundColors = props.statuses.map(s => getColorFromId(s.id, 0.5));
const borderColors = props.statuses.map(s => getColorFromId(s.id, 1));
...
const chartData = ref({
  labels: props.statuses.map(s => s.name),
  datasets: [{
    label: 'Cantidad de Artículos',
    data: props.statuses.map(s => s.total),
...
const chartOptions = ref({
  ...
  plugins: {
    legend: { position: 'top' },
    title: { display: true, text: 'Distribución de Artículos por Estatus' }
  },
  scales: {
...
<template>
  <HeadLogo title="Artículos Por Estatus" />
  <LayoutAuthenticated>
    <SectionMain>
      <SectionTitleLineWithButton
        :icon="mdiChartTimelineVariant"
        title="Artículos Por Estatus"
        main
      />
      <div id="charts-pdf-content" class="p-4 sm:p-6 bg-white dark:bg-gray-800 rounded-lg shadow relative">
        ...
        <div class="mt-8 overflow-x-auto shadow-md rounded-lg">
          <h2 class="text-xl font-semibold mb-4 text-gray-800 dark:text-gray-200">Datos detallados por estatus</h2>
          <table class="min-w-full border border-gray-200 dark:border-gray-700">
            ...
            <tbody>
              <tr v-for="(status, index) in props.statuses" :key="index" :class="{'bg-gray-50 dark:bg-gray-700': index % 2 === 0, 'bg-white dark:bg-gray-800': index % 2 !== 0}">
                ...
                <td class="px-4 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300 border-b border-gray-200 dark:border-gray-700">
                  {{ Math.round((status.total / props.statuses.reduce((a, b) => a + b.total, 0)) * 100) }}%
                </td>
              </tr>
              <tr class="bg-gray-100 dark:bg-gray-600 font-semibold">
                ...
                <td class="px-4 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100 border-b border-gray-200 dark:border-gray-700">
                  {{ props.statuses.reduce((a, b) => a + b.total, 0) }}
                </td>
                <td class="px-4 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100 border-b border-gray-200 dark:border-gray-700">
                  100%
                </td>
