...
const form = useForm({ ...props.criteria });
const saveForm = () => {
    form.put(route(`${props.routeName}update`, props.criteria.id));
};
const deleteForm = () => {
    Swal.fire({
        title: "¿Esta seguro?",
        ...
        if (res.isConfirmed) {
            form.delete(route(`${props.routeName}destroy`, props.criteria.id));
        }
    });
};
</script>
...
<template>
    ...
    <LayoutMain>
        ...
        <CardBox isForm @submit.prevent="saveForm">
            <FormField label="Nombre:" :required="true" help="Nombre del módulo" :error="form.errors.name">
            ...
            <FormField label="Descripción:" :required="true" help="" :error="form.errors.description">
                <FormControl height="h-28" type="textarea" v-model="form.description" placeholder="Descripción" />
            </FormField>
