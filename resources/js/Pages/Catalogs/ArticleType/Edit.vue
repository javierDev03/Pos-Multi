...
import { mdiPencil, mdiTrashCan, mdiContentSave, mdiClose, mdiPlus } from '@mdi/js'
...
const form = useForm({ ...props.articleType })

const saveForm = () => {
    form.put(route(`${props.routeName}update`, props.articleType.id))
}
...
const deleteForm = () => {
    Swal.fire({
        ...
        if (res.isConfirmed) {
            form.delete(route(`${props.routeName}destroy`, props.articleType.id))
        }
    })
}

// lismits for rubric items and answers
const addItem = (rubricIndex) => {
    if (form.rubrics[rubricIndex].items.length < 10) {
        form.rubrics[rubricIndex].items.push({ question: '', answers: [] })
    }
}
const addAnswer = (rubricIndex, itemIndex) => {
    if (form.rubrics[rubricIndex].items[itemIndex].answers.length < 5) {
        form.rubrics[rubricIndex].items[itemIndex].answers.push({ text: '', score: 0 })
    }
}
...
<template>
    ...
    <LayoutMain>
        ...
        <CardBox isForm @submit.prevent="saveForm">
            <FormField label="Nombre del Tipo de Artículo:" :required="true" help="Nombre del tipo"
            ...
            <div class="mt-8">
                <div v-for="(rubric, rubricIndex) in form.rubrics" :key="rubricIndex" class="mb-6">
                    <CardBox class="bg-gray-50">
                        ...
                        <FormField :label="`Título de la Rúbrica ${rubricIndex + 1}:`" :required="true"
                            :error="form.errors[`rubrics.${rubricIndex}.title`]">
                            <FormControl v-model="rubric.title" placeholder="Ej: Criterios de Evaluación General" />
                        </FormField>
                        ...
                        <div class="mt-4">
                            <div class="flex justify-between items-center mb-3">
                                ...
                                <div>
                                    <BaseButton v-if="rubric.items.length < 10" @click="addItem(rubricIndex)"
                                        :icon="mdiPlus" color="success" label="Agregar Pregunta" small outline />
                                    <p v-else class="text-xs text-gray-500 italic">Máximo 10 preguntas alcanzado</p>
                                </div>
                            </div>

                            <div v-for="(item, itemIndex) in rubric.items" :key="itemIndex"
                                class="mb-4 p-4 border border-gray-200 rounded">
                                <div class="flex justify-between items-center mb-2">
                                    <h6 class="text-sm font-medium">Pregunta {{ itemIndex + 1 }}</h6>
                                    <BaseButton @click="removeItem(rubricIndex, itemIndex)" :icon="mdiTrashCan"
                                        color="danger" small outline />
                                </div>

                                <FormField :label="`Pregunta ${itemIndex + 1}:`" :required="true"
                                    :error="form.errors[`rubrics.${rubricIndex}.items.${itemIndex}.question`]">
                                    <FormControl v-model="item.question"
                                ...
                                <div class="mt-3">
                                    <div class="flex justify-between items-center mb-2">
                                        ...
                                        <div>
                                            <BaseButton v-if="item.answers.length < 5"
                                                @click="addAnswer(rubricIndex, itemIndex)" :icon="mdiPlus" color="info"
                                                label="Agregar Respuesta" small outline />
                                            <p v-else class="text-xs text-gray-500 italic">Máximo 4 respuestas alcanzado
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    <div v-for="(answer, answerIndex) in item.answers" :key="answerIndex"
                                        ...
                                        <div class="w-24">
                                            <FormField label="Puntaje:"
                                                ...
                                                <FormControl v-model="answer.score" type="number" step="0.1"
                                                    placeholder="0.0" />
                                            </FormField>
                                        </div>
                                        <BaseButton @click="removeAnswer(rubricIndex, itemIndex, answerIndex)"
                                            :icon="mdiTrashCan" color="danger" small outline />
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <template #footer>
                <BaseButtons>
                    <BaseButton :href="route(`${routeName}index`)" :icon="mdiClose" color="white" label="Cancelar" />
