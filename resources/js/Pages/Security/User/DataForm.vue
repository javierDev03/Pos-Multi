...
const areasData = propsData.areas.filter(area => area.parent_id === null) ?? []
...
const filterAreas = () => {
    if (form.selectedArea !== propsData.user?.knowledge_area?.parent_id) {
        form.knowledge_area_id = null
    }
    subareasData.value = propsData.areas.filter(area => area.parent_id === form.selectedArea)
}
...
<template>
    ...
    <div class="md:flex md:space-x-4 my-5">
        ...
        <div class="md:w-1/2">
            <FormField label="SubArea:" :error="form.errors.knowledge_area_id">
    ...
    <FormField required label="Institución:" :error="form.errors.institution_id" class="dark:text-white">
        <Select placeholder="Busca una institución" :itemSelected="propsData?.user?.institution != null ? propsData?.user?.institution : null" v-model="form.institution_id" required :options="results"
