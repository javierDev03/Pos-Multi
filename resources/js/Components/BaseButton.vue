...
const componentClass = computed(() => {
  const base = [
    ...
    "border", props.disabled || props.procesing ? "cursor-not-allowed" : "cursor-pointer",
    props.roundedFull ? "rounded-full" : "rounded",
    getButtonColor(props.color, props.outline, !(props.disabled || props.procesing), props.active),
  ];
  ...
  if (props.disabled || props.procesing) {
...
<template>
  <component
    ...
    :type="computedType"
    :target="target"
    :disabled="props.disabled || props.procesing">
    
    <BaseIcon v-if="icon" :path="icon" :size="iconSize" :class="iconColor" />
    <span v-if="label" :class="labelClass">{{ label }}</span>
    
    <svg v-if="props.procesing" class="animate-spin h-5 w-5 text-white ml-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
